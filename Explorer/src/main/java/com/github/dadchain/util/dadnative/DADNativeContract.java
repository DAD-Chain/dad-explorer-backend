...
public class DADNativeContract {
    ...
    public static final String DISTRIBUTE_BONUS = "distributeBonus";
     ...
     * @param token
     * @param quantity
     * @param userBonusRatio
     * @param publisherBonusRatio
     * @param publisherID
     * @param userAddresses
     * @param gaslimit
     * @param gasprice
     ...
     * @throws Exception
     */
    public String distributeBonus(Account account,
                                  String orderID,
                                  String adType,
                                  long price,
                                  String token,
        ...
        List list = new ArrayList();
        list.add(new Struct().add(orderID, adType, price, token
                , quantity, userBonusRatio, publisherBonusRatio, publisherID, userAddresses));
        byte[] args = NativeBuildParams.createCodeParamsScript(list);
        Transaction tx = sdk.vm().buildNativeParams(new Address(Helper.hexToBytes(contractAddress)),
                DISTRIBUTE_BONUS, args, payerAcct.getAddressU160().toBase58(), gaslimit, gasprice);
        sdk.signTx(tx, new Account[][]{{account}});
        if (!account.equals(payerAcct)) {
            sdk.addSign(tx, payerAcct);
        }
