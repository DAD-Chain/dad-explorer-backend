...
public class CommonService {
    ...
    public void batchInsertDb(int tHeight, BatchBlockDto batchBlockDto) {
        //插入tbl_address_txs表
        if (batchBlockDto.getAddressTxs().size() > 0) {
            int count = batchBlockDto.getAddressTxs().size();
            if (count > paramsConfig.BATCHINSERT_SQL_COUNT) {
                for (int j = 0; j <= count / paramsConfig.BATCHINSERT_SQL_COUNT; j++) {
                    List<AddressTxs> list = batchBlockDto.getAddressTxs().subList(j * paramsConfig.BATCHINSERT_SQL_COUNT, (j + 1) * paramsConfig.BATCHINSERT_SQL_COUNT > count ? count : (j + 1) * paramsConfig.BATCHINSERT_SQL_COUNT);
                    if (list.size() > 0) {
                        addressTxsMapper.batchInsert(list);
                    }
                }
            } else {
                addressTxsMapper.batchInsert(batchBlockDto.getAddressTxs());
            }
        }
        ...
        // 更新每日交易量
        Integer newAddressCount = 0;
        if (batchBlockDto.getTxDetails().size() > 0) {
            Long lastDay = 0L;
            for (Block blk : batchBlockDto.getBlocks()) {
                if (blk.getTxCount() > 0) {
                    Long blkDay = blk.getBlockTime() / ConstantParam.DAY_IN_SECONDS;
                    if (lastDay.compareTo(blkDay) != 0) {
                        txDailySummaryMapper.IncDailyTxCount(blkDay, blk.getTxCount());
                        lastDay = blkDay;
                    }
                    Set<String> addresses = batchBlockDto.getBlkAddresses().get(blk.getBlockHeight());
                    if (null != addresses && addresses.size() > 0) {
                        List<AddressAll> binst = addresses.stream().map((addr) -> new AddressAll(addr, blkDay.intValue())).collect(Collectors.toList());
                        newAddressCount += addressAllMapper.batchInsert(binst);
                    }
                }
            }
        }
        ...
        Current current = Current.builder()
                .blockBookkeeperCount(ConstantParam.BLOCK_BOOKKEEPER_COUNT)
                .blockHeight(tHeight)
    ...
    /**
     * get the the blockheight of the dad blockchain
     *
     * @return
     * @throws Exception
     */
